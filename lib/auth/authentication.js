...
var getExpirationDate = function(timeStamp) {
    ...
    return nowDateTimeStamp.getTime() + parseInt(timeStamp) * 1000;
};
...
var checkValidAndRefreshToken = function(request, response, next) {
    ...
    getAccessTokenUsingRefreshTokenFlow(Authentication.AUTH_OPTIONS, request._passport.session.user, function(err, res){
        if (err) {
            return next(err);
        }
        res = JSON.parse(res);
...
Authentication.prototype.use = function (options) {
    ...
    Authentication.AUTH_OPTIONS = _.clone(this.authOptions);
