...
var proxyRequest = function (request, response) {
    ...
    console.log('[PROXY REQUEST ACCESS TOKEN]', request._passport.session.user.accessToken);
    console.log('[PROXY REQUEST EXPIERS]',
        (new Date(request._passport.session.user.expires_in)), new Date(), request._passport.session.user.expires_in - Date.now());
